...
public class VaccinationCenterFinal {
    ...
    static int AZCount = 0;     //AstraZeneca vaccine count.
    static int SPCount = 0;     //Sinopharm vaccine count.
    static int PFCount = 0;     //Pfizer vaccine count.
    ...
    public static void main(String[] args) throws IOException {
        ...
        //declaring 3 LinkedLists for separate vaccines. AZ - AstraZeneca, SP - Sinopharm, PF - Pfizer
        LinkedList<Patient> waitingListAZ = new LinkedList<>();
        LinkedList<Patient> waitingListSP = new LinkedList<>();
        LinkedList<Patient> waitingListPF = new LinkedList<>();
        ...
        while (true) {
            ...
            //add a patient to the booth.
                ...
                if (vaccineType.equalsIgnoreCase("AstraZeneca")) {              //checking the vaccine type to direct the patient to the correct booth.
                    if (AZCount < 2) {
                        for (int i = 0; i < 2; i++) {
                        ...
                        System.out.println("Sorry! There are no any empty booths. You will be added to a waiting list!");
                        waitingListAZ.add(new Patient(firstName,surname,age,city,NIC,vaccineType));
                    }
                }

                else if (vaccineType.equalsIgnoreCase("Sinopharm")) {                   //checking the vaccine type to direct the patient to the correct booth.
                    if (SPCount < 2) {
                        for (int i = 2; i < 4; i++) {
                            if (PatientData[i].getFirstName().equalsIgnoreCase("empty")) {
                                System.out.println("The booth"+" "+i+" "+"is"+" "+PatientData[i].getFirstName());
                        ...
                        System.out.println("Sorry! There are no any empty booths. You will be added to a waiting list!");
                        waitingListSP.add(new Patient(firstName,surname,age,city,NIC,vaccineType));
                    }
                }

                else if (vaccineType.equalsIgnoreCase("Pfizer")) {                      //checking the vaccine type to direct the patient to the correct booth.
                    if (PFCount < 2) {
                        for (int i = 4; i < 6; i++) {
                            if (PatientData[i].getFirstName().equalsIgnoreCase("empty")) {
                                System.out.println("The booth"+" "+i+" "+"is"+" "+PatientData[i].getFirstName());
                        ...
                        System.out.println("Sorry! There are no any empty booths. You will be added to a waiting list!");
                        waitingListPF.add(new Patient(firstName,surname,age,city,NIC,vaccineType));
                    }
                }
            }
            ...
            //remove a patient from a booth.
                ...
                System.out.println(VaccinationCenter[boothNum].getfirstName()+" "+"is removed. The next patient will be directed to the booth.");
                ...
                if(boothNum == 0 || boothNum == 1){
                    AZCount--;
                    PatientData[boothNum] = waitingListAZ.getFirst();
                    VaccinationCenter[boothNum].setfirstName(PatientData[boothNum].getFirstName());
                    waitingListAZ.removeFirst();
                    vaccineAmount = vaccineAmount - 1;
                }
                else if(boothNum == 2 || boothNum == 3){
                    SPCount--;
                    PatientData[boothNum] = waitingListSP.getFirst();
                    VaccinationCenter[boothNum].setfirstName(PatientData[boothNum].getFirstName());
                    waitingListSP.removeFirst();
                    vaccineAmount = vaccineAmount - 1;
                }
                else if(boothNum == 4 || boothNum == 5){
                    PFCount--;
                    PatientData[boothNum] = waitingListPF.getFirst();
                    VaccinationCenter[boothNum].setfirstName(PatientData[boothNum].getFirstName());
                    waitingListPF.removeFirst();
                    vaccineAmount = vaccineAmount - 1;
                }
